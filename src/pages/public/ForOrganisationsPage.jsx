    ...
    <PublicLayout>
      ...
      <section style={{ background: '#0A0A0A', ...SGF }} className="py-28 px-6 text-center">
        <div className="max-w-3xl mx-auto">
          ...
          <p style={{ fontSize: 20, color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, maxWidth: 500, margin: '0 auto 36px', ...SGF }}>
      ...
      <section style={{ background: '#111111', ...SGF }} className="py-20 px-6">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center mb-14" style={{ fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 700, color: '#FFFFFF', ...SGF }}>
          ...
          <div className="flex flex-col gap-5">
            {symptoms.map((s) => (
              ...
              >
                ...
                >
                  <XIcon size={18} style={{ color: 'rgba(255,255,255,0.5)' }} />
                </div>
                <p style={{ fontSize: 22, fontWeight: 700, color: '#FFFFFF', ...SGF }}>{s}</p>
              </div>
            ))}
          </div>
          <div
          ...
          >
            <p style={{ fontSize: 19, fontWeight: 700, color: '#C7F24D', ...SGF }}>These are system problems, not people problems.</p>
            <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', marginTop: 6, ...SGF }}>The fix is the operating model.</p>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#0A0A0A', ...SGF }} className="py-24 px-6">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-center mb-16" style={{ fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 700, color: '#FFFFFF', ...SGF }}>
                ...
                <div key={p.step} className="flex gap-8 pb-10 relative">
                  <div
                    ...
                    style={{
                      ...
                      color: idx === 0 ? '#0A0A0A' : 'rgba(255,255,255,0.7)',
                      fontSize: 15,
                      fontWeight: 800,
                      ...SGF,
                    }}
                  ...
                  <div style={{ paddingTop: 12, flex: 1 }}>
                    <h3 style={{ fontSize: 22, fontWeight: 700, color: '#FFFFFF', marginBottom: 6, ...SGF }}>{p.title}</h3>
                    <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, ...SGF }}>{p.body}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section style={{ background: '#111111', ...SGF }} className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-center mb-12" style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: 700, color: '#FFFFFF', ...SGF }}>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t) => (
              ...
              >
                ...
                <p style={{ fontSize: 17, color: 'rgba(255,255,255,0.8)', lineHeight: 1.7, marginTop: -20, ...SGF }}>{t.quote}</p>
                <div className="mt-auto pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                  <p style={{ fontWeight: 700, color: '#FFFFFF', fontSize: 16, ...SGF }}>{t.name}</p>
                  <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', ...SGF }}>{t.role}, {t.company}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: '#1A1A1A', ...SGF }} className="py-24 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          <h2 style={{ fontSize: 'clamp(30px, 4vw, 52px)', fontWeight: 800, color: '#FFFFFF', marginBottom: 12, lineHeight: 1.2, ...SGF }}>
            Let's look at your team together
          </h2>
          <p style={{ fontSize: 20, color: 'rgba(255,255,255,0.7)', marginBottom: 36, lineHeight: 1.7, ...SGF }}>
            Book a 45-minute discovery call. No sales pitch — just an honest assessment.
          </p>
          <Link
            to="/contact"
            className="inline-flex items-center gap-2 font-semibold transition-all"
            style={{ background: '#C7F24D', color: '#0A0A0A', borderRadius: 999, padding: '16px 36px', fontSize: 17, ...SGF }}
          >
            Book a discovery call <ArrowRight size={16} />
          </Link>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', marginTop: 16, ...SGF }}>Responses within one business day</p>
        </div>
      </section>
    </PublicLayout>
  )
